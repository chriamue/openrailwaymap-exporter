...
use geo::Distance as GeoDistance;
use geo::{Coord, Euclidean, Haversine, Point};
...
impl Distance for Coord<f64> {
    fn distance(&self, other: &Coord<f64>) -> Length {
        ...
        let haversine_distance = Haversine.distance(point1, point2);
...
impl Distance for Point<f64> {
    fn distance(&self, other: &Point<f64>) -> Length {
        let euclidean_distance = Euclidean.distance(*self, *other);
