...
async fn when_call_position_on_edge(world: &mut BddWorld) {
    ...
    let current_location = world.current_location.expect("Current location is not set");
    let distance_to_travel = world
        .distance_to_travel
        .expect("Distance to travel is not set");
...
async fn then_new_position_approximately(world: &mut BddWorld, x: f64, y: f64) {
    ...
    assert_relative_eq!(new_position.x, expected_position.x, epsilon = 0.01);
    assert_relative_eq!(new_position.y, expected_position.y, epsilon = 0.01);
}
